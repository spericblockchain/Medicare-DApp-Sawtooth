    ...
    class Client {
        ...
        async sendData(payload) {
            ...
            let flag = 0;
            if(p.action === 'addNewDoctor'){
                console.log("TCL: Client -> sendData -> p.id", p.id)
                const doctor = await this.getDoctors(p.id)
                console.log("TCL: Client -> sendData -> doctor", doctor)
                const data = await JSON.parse(doctor);
                if(data && data.publickey === p.id){
                    flag = 1
                    console.log("TCL: Client -> sendData -> flag", flag)
                }
            }
            else if(p.action === 'updateDoctor'){
                const doctor = await this.getDoctors(p.id)
                console.log("TCL: Client -> sendData -> doctor", doctor)
                const data = await JSON.parse(doctor);
                if(data && data.error){
