    ...
    class Client {
            ...
            try {
                ...
                if(Array.isArray(responseJson.data)) {
                    ...
                    for(let hospital of responseJson.data) {
                        hospitals.push(new Buffer(hospital.data,'base64').toString());
                    }                
        ...
        async sendData(payload) {
            ...
            let flag = 0;
            if(p.action === 'addNewHospital'){
                console.log("TCL: Client -> sendData -> p.id", p.id)
                hospitals = await client.getHospitals(p.id)
                console.log("TCL: Client -> sendData -> hospitals", hospitals)
                const data = await JSON.parse(hospitals);
                if(data && data.publickey === p.id){
                    flag = 1
                    console.log("TCL: Client -> sendData -> flag", flag)
                }
            }
            else if(p.action === 'updateHospital'){
                hospitals = await client.getHospitals(p.id)
                console.log("TCL: Client -> sendData -> hospitals", hospitals)
                const data = await JSON.parse(hospitals);
                if(data && data.error){
            ...
            const transactionHeaderBytes = await protobuf.TransactionHeader.encode({
            ...
            const transaction = await protobuf.Transaction.create({
                header: transactionHeaderBytes,
                headerSignature: this.signer.sign(transactionHeaderBytes),
                payload: payloadBytes
            });
            const transactions = [transaction];
            const batchHeaderBytes = await protobuf.BatchHeader.encode({
                signerPublicKey: this.publicKey,
                transactionIds: transactions.map((txn) => txn.headerSignature),
            }).finish();
            const batch = await protobuf.Batch.create({
                header: batchHeaderBytes,
                headerSignature: this.signer.sign(batchHeaderBytes),
                transactions: transactions,
            });
            const batchListBytes = await protobuf.BatchList.encode({
                batches: [batch]
            }).finish();
